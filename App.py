...
def main():
    ...
    if save_record_button:
        ...
        # try:
        #     # Read existing data from the Excel file
        #     df = pd.read_excel(file_path, sheet_name='InvoiceLogTemplate')
            
        #     # Convert add_new_record to DataFrame and append to df
        #     new_record_df = pd.DataFrame([add_new_record])
        #     df = pd.concat([df, new_record_df], ignore_index=True)
            
        #     # Write the updated DataFrame back to the same sheet in the Excel file
        #     with pd.ExcelWriter(file_path, mode='w', engine='openpyxl') as writer:
        #         df.to_excel(writer, index=False, sheet_name='InvoiceLogTemplate')
